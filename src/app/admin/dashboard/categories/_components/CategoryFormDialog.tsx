		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
				<DialogHeader>
					...
					<DialogDescription>
						The English name is required. German is optional, but a category
						without it falls back to English on the German site.
					</DialogDescription>
				</DialogHeader>

				{/* Keyed on the category so switching rows rebuilds the form with the
				    right defaults — useForm only reads defaultValues once. */}
				<ProForm
					key={category?.id ?? "new"}
					onSubmit={onSubmit}
